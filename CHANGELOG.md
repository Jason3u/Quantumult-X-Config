...
## 2026-09-06（六）· 订阅临时切换良心云后回退（仅 QX，Clash 未动）

- 曾短暂切换到良心云（tag=良心云），用户端下载配置失败（服务端 raw/jsdelivr 已验证为新内容，判断为用户网络问题），已回退
- 现状：吹雪云为生效订阅；良心云 URL 注释保留备用（QX 原生 vless 格式无需 opt-parser，含 5 个香港住宅IP节点，无台湾/韩国节点）
- Telegram固定节点 / Cornix固定节点 的 resource-tag-regex 保持 `^吹雪云$`

## 2026-09-06（五）· Telegram 专属分流（消息延迟根治）

### 排查结论
用户反馈「TG 消息不及时、每次点进去要刷新一段时间，Cornix 一样」。排查发现 **TG 此前完全没有规则和策略组**，且存在一个关键盲区：
- **TG App 收发消息直连数据中心 IP，不查 DNS** → 配置里全部 host-suffix 域名规则永远匹配不上 App 核心流量
- TG 流量一直掉进兜底分流 → 香港自动 →（空的香港住宅IP组）→ 香港节点 url-test：每次新连接落在测速组当时选中的节点上，晚高峰节点拥塞/丢包时 MTProto 拉消息就卡
- Cornix 同理：WebSocket 每次重连落在测速组选中的节点，该节点到欧美后端线路差就转圈

### 修改
- **新增 Telegram 策略组**：`static=Telegram, 香港节点, 日本节点, 新加坡节点, 台湾节点, 韩国节点, 美国节点`（默认香港节点，直挂测速组绕开空住宅链路；出口固定、可手动钉节点。TG 亚洲主 DC 在新加坡，晚高峰可手动切日/新对比）
- **新增 Telegram 规则引用**（update-interval=3600）：规则文件见规则仓库 `qx/Telegram.list`——22 个域名 + TG 官方 DC IP 段（149.154.160.0/20、91.108.0.0/16 等 6 段 IPv4 + 4 段 IPv6），ip-cidr 规则确保 App 直连 IP 的核心流量也被接管
- **Cornix 组扩充备选**：[香港节点, 新加坡节点, 日本节点] → 增加 台湾/韩国/美国节点（默认香港不变，晚高峰可钉日/新）
- Clash 配置（ClashForSelf.yaml）同步：Telegram select 组 + rule-provider + RULE-SET（IP-CIDR 带 no-resolve），Cornix 组同步扩充
